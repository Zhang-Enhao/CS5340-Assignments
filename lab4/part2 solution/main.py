...
def factor_evidence_update(factors, evidence):
    """
    Updates the factors with the evidence and returns a new factor dictionary.
    """
    for factor_index in factors:
        factors[factor_index] = factor_evidence(factors[factor_index], evidence)
    return factors

def initialize_samples_and_nodes(initial_samples, evidence, nodes):
    """
    Initializes the samples and nodes by removing the evidence nodes from the samples and nodes.
    """
    for node in evidence:
        if node in initial_samples:
            initial_samples.pop(node)
        if node in nodes:
            nodes = np.delete(nodes, np.argwhere(nodes == node))
    return initial_samples, nodes

def run_sampling(total_run, nodes, factors, initial_samples):
    """
    Runs the sampling for the specified number of runs and returns the samples.
    """
    sample_result = np.zeros((total_run, len(nodes)))
    for i in range(total_run):
        initial_samples = _sample_step(nodes, factors, initial_samples)
        sample_result[i] = list(initial_samples.values())
    return sample_result

def compute_frequency(sample_result, num_burn_in, num_iterations, card):
    # Initialize the frequency dictionary with all possible indices as keys
    freq = {i: 0 for i in range(np.prod(card))}
    for i in range(num_burn_in, num_iterations):
        index = assignment_to_index(sample_result[i], card)
        if index in freq:
            freq[index] += 1
        else:
            # Initialize the missing key with a count of 1
            freq[index] = 1
    return freq


def frequency_to_probability(freq):
    freq_arr = np.array(list(freq.values()))
    return freq_arr / np.sum(freq_arr)
...
def _sample_step(nodes, factors, in_samples):
    ...
    for i in range(len(nodes)):
        node = nodes[i]
        factor = factors[node]
        node_card = factor.card[i]
        # Create an array of probabilities for each possible value of the node
        ...
        p /= p.sum()
        # Sample a new value for the node based on the probabilities
        samples[node] = np.random.choice(range(node_card), p=p)
...
def _get_conditional_probability(nodes, edges, factors, evidence, initial_samples, num_iterations, num_burn_in):
    ...
    """ YOUR CODE HERE """
    factors = factor_evidence_update(factors, evidence)
    initial_samples, nodes = initialize_samples_and_nodes(initial_samples, evidence, nodes)

    total_run = num_burn_in + num_iterations
    sample_result = run_sampling(total_run, nodes, factors, initial_samples)

    freq = compute_frequency(sample_result, num_burn_in, num_iterations, factors[0].card)
    freq_arr = frequency_to_probability(freq)
